...
SUPPRESS_POWER = 2      # intensity points removed by one suppress action (1-5 scale)
PREP_POWER = 1          # catchability points removed by one prep action (1-5 scale)
...
def apply_commands(
    ...
    Each command costs 1 action point. Every crew does the same fixed work.

    Actions:
      move      - move to adjacent cell [x,y]
      suppress  - reduce intensity of current burning cell by SUPPRESS_POWER
      prep      - reduce catchability of current cell by PREP_POWER
      evacuate  - carry up to EVACUATE_CAPACITY people to adjacent cell [x,y]; crew moves with them
    """
